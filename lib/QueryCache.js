...
QueryCache.prototype.invalidate = function() {
  ...
  function update() {
      console.log("Appel de update pour la requette ", self.query);
    ...
    self.base.db.query(self.query, function(error, rows) {
      ...
      if(rows.length === 0 && self.initialized === false) {
        ...
        console.log("Difference entre les jeux de données:", diff);
        if(diff !== null) {
  ...
  console.log("minInterval:", self.base.settings.minInterval, typeof self.base.settings.minInterval);
  console.log("lastUpdate:", self.lastUpdate, typeof self.lastUpdate);
  console.log("test if update time:", self.lastUpdate + self.base.settings.minInterval < Date.now());
  if(typeof self.base.settings.minInterval !== 'number') {
      console.log("update time no minInterval");
    update(); 
  } else if(self.lastUpdate + self.base.settings.minInterval < Date.now()) {
      console.log("update time after minInterval");
    update();
