...
function LiveMysql(settings, callback){
  ...
  //the delay we give the database to fully apply latest commit:
  var delayBeforeCacheInvalidationQuery = settings.delayBeforeCacheInvalidationQuery;
  if( typeof delayBeforeCacheInvalidationQuery != "number" || delayBeforeCacheInvalidationQuery < 0)
    delayBeforeCacheInvalidationQuery = 0;
      ...
      Object.keys(self._queryCache).forEach(function(query) {
        ...
        if((self.settings.checkConditionWhenQueued
          ...
          && curCache.matchRowEvent(event)){
            //We wait some time before performing the cache invalidation, to make sure
            //latest commit was fully applied:
            setTimeout(curCache.invalidate.bind(curCache), delayBeforeCacheInvalidationQuery);
        }
      });
    })
